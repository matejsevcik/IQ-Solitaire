...
public class Node {
	...
	boolean canJumpUp(int row, int column) {
		if(row == 0 || row == 1)
			return false;
		if(charAt(row-1, column) == '0' && charAt(row-2, column) == '0')
			return true;
		else
			return false;
	}
	...
	boolean canJumpLeft(int row, int column) {
		if(column == 0 || column == 1)
			return false;
		if(charAt(row, column-1) == '0' && charAt(row, column-2) == '0')
			return true;
		else
			return false;
	}
